...
from models import Base, User, Report, ReportType, UserRole, AuditLog, SiteLocation
...
def startup_tasks():
    ...
    if not db.query(User).filter(User.username == "admin").first():
        db.add(User(username="admin", hashed_password=pwd_context.hash("password123"), role=UserRole.ADMIN, site_location=SiteLocation.GLOBAL.value))
        db.commit()
    db.close()
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    return {"access_token": create_access_token(data={"sub": user.username}), "token_type": "bearer", "role": user.role.value, "site": user.site_location}

@app.post("/users/")
def create_user(username: str = Form(...), password: str = Form(...), role: UserRole = Form(...), site: str = Form(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_report_summary(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if user.role != UserRole.ADMIN: base_query = base_query.filter(Report.site_location == user.site_location)
...
def list_reports(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for r in all_reports:
        ...
        if r.report_type == ReportType.SAFEGUARDING and user.role not in [UserRole.DSL, UserRole.ADMIN] and r.reporter_id != user.id: continue
...
def close_report(
    ...
    rca_1: str = Form(""), rca_2: str = Form(""), rca_3: str = Form(""), rca_4: str = Form(""), rca_5: str = Form(""),
...
def get_audit_logs(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    logs = db.query(AuditLog, User.username).outerjoin(User, AuditLog.user_id == User.id).order_by(AuditLog.timestamp.desc()).limit(100).all()
    return [{"timestamp": l[0].timestamp, "username": l[1] if l[1] else "System/External", "action": l[0].action} for l in logs]
